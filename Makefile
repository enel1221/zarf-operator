# Version for image tags and chart
VERSION ?= 0.1.0

# Image URL to use all building/pushing image targets
IMG ?= ghcr.io/enel1221/zarf-operator:v$(VERSION)

# Sidecar image URL
SIDECAR_IMG ?= ghcr.io/enel1221/zarf-operator-sidecar:v$(VERSION)

# OCI registries for helm and zarf
HELM_REGISTRY ?= ghcr.io/enel1221/charts
ZARF_REGISTRY ?= oci://ghcr.io/enel1221/packages
...
.PHONY: helm-push
helm-push: helm-package ## Push the Helm chart to OCI registry.
	helm push dist/zarf-operator-$(VERSION).tgz oci://$(HELM_REGISTRY)

.PHONY: zarf-package
zarf-package: ## Build the Zarf package.
	cd dist && zarf package create . --confirm --output .

.PHONY: zarf-publish
zarf-publish: zarf-package ## Publish the Zarf package to OCI registry.
	cd dist && zarf package publish zarf-package-zarf-operator-*.tar.zst $(ZARF_REGISTRY)
